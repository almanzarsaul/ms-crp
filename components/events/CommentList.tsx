...
// Assuming the getEventLikes and getUser methods are already implemented elsewhere
import {
  addCommentToEvent,
  getEventComments,
  getEventLikes,
} from "@/models/Event"; // Adjust this import according to your file structure
import { getUser } from "@/models/User"; // Adjust this import according to your file structure
import { User } from "@/models/types/User";
import { Comment } from "@/models/types/Event";
...
const CommentList = ({ eventId }: { eventId: string }) => {
  ...
  const handleComment = async () => {
    await addCommentToEvent(eventId, content, currentUser);
    fetchComments();
    setContent("");
  };
  ...
  const fetchComments = async () => {
    try {
      ...
      const commentsList = await getEventComments(eventId);

      if (commentsList.length === 0) {
        console.log("No comments found for this event.");
      }
